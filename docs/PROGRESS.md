...
Prepare R1, the durable MQTT-to-stream-catalog vertical slice. Before implementation, record decisions for the authoritative stream-catalog store, telemetry observation retention, MQTT topic authorization and scope, stream identity and idempotency, and malformed payload and redelivery handling.
