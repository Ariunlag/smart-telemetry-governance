...
**R2 manual class manager backend slice:** tenant-scoped manual telemetry-class CRUD, stream membership, and saved declarative class-query definition storage are implemented with migration-backed PostgreSQL persistence, bounded validation, and class-owned deletion cleanup. Tenant isolation is enforced for classes, memberships, and query definitions. Deleting a class preserves streams, source/site ownership, raw observations, processing tasks, and outbox records. Saved class queries are definitions only: historical InfluxDB query execution, live MQTT/WebSocket append, visualization frontend, duplicate grouping, automatic class recommendations, embeddings, and RAG are not implemented. R2 remains in progress and is not production-ready.
