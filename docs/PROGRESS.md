...
**R2 -- Schema-flexible interoperability foundation:** its initial protocol-neutral boundary is implemented under [ADR-005](decisions/ADR-005-schema-flexible-interoperability.md) and the proposed [R2 threshold](evaluation/thresholds/r2.yaml). MQTT is the only active runtime adapter; it emits immutable bounded `RawObservation` evidence and maps its topic to the generic external stream identifier before the existing catalog/evidence/outbox flow. No schema observation, field profiles, fingerprints, versions, drift evidence, second adapter, mappings, or AI is implemented. R2 remains in progress and is not production readiness.
