...
The authoritative record is a relational governance store plus immutable/raw evidence references. PostgreSQL owns the catalog, governance metadata, provenance, evidence metadata, and future observation-delivery outbox. A future InfluxDB projection may store normalized accepted observations for time-series use, but is not a catalog and must be delivered through the ADR-004 transactional-outbox boundary. Derived views and model outputs must be rebuildable and must never overwrite raw observation or human decision history.

The initial R1 slice implements the [ADR-003](../decisions/ADR-003-r1-stream-catalog-entry-decisions.md) constraints for PostgreSQL catalog authority, bounded evidence, MQTT authorization, stream identity/idempotency, and malformed/redelivery handling. [ADR-004](../decisions/ADR-004-influx-observation-delivery.md) defines the remaining R1 InfluxDB observation-delivery boundary. InfluxDB is not the catalog, and ChromaDB, Neo4j, and `pgvector` are not used in R1.
