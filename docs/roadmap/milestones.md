...
### Current source/subscription persistence boundary
Migration-backed PostgreSQL records now establish tenant/site-aware persistence for tenants, sites, telemetry sources, MQTT subscriptions, and ingestion runs. Database constraints preserve ownership consistency, prohibit an unrestricted `#` subscription, constrain MQTT QoS and persisted configuration values, prevent duplicate active subscriptions, and bound run counters and error codes. Source credential fields contain only opaque external references. This is a persistence foundation only: source/subscription APIs, credential retrieval, and MQTT runtime orchestration are not implemented.
...
Accepted raw observations are durable PostgreSQL records linked to stream evidence, with a retention timestamp and independent generic processing-task state. Accepted JSON observations atomically queue `schema_observation / r2.schema.v1` tasks. The setting-controlled background worker claims bounded batches, persists deterministic structural schema fingerprints, versions, fields, drift events, and provenance, then completes tasks atomically. Structural failures transition to dead-letter; transient failures use bounded exponential retry and dead-letter at the attempt limit. Raw evidence remains authoritative, and persisted schema metadata excludes source scalar values. Physical raw-payload cleanup remains pending.
The Manual Class Manager backend slice is implemented: tenant-scoped telemetry-class CRUD, manual stream membership, and saved declarative class-query storage use migration-backed PostgreSQL persistence with bounded validation and class-owned deletion cleanup. Class deletion preserves stream/source/site ownership, raw observations, processing tasks, and outbox records. Saved queries are definitions only; historical InfluxDB query execution, live MQTT/WebSocket append, visualization frontend, duplicate grouping, automatic class recommendations, embeddings, RAG, semantic mappings, and review UI are not implemented. R2 remains in progress and is not production readiness.
### Required tests
