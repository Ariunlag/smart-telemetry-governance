...
### Remaining R1 work
PR #9 merged the PostgreSQL-outbox-to-InfluxDB delivery boundary at `f7b8c419d4b1851dee453011259dc381ea6f08c4`; GitHub Actions CI passed. Broader R1 remains in progress and is not production readiness. The next implementation gate is the proposed [R1 source/subscription threshold](../evaluation/thresholds/r1.yaml): tenant/site ownership, governed source registration, external credential references, subscription control, ingestion-run evidence, retained-message policy, broker-backed validation, and retention enforcement. Classification, embeddings, unit inference, quality scoring, duplicate detection, clustering, RAG, agents, graph infrastructure, and anomaly detection remain out of scope for this R1 target.
